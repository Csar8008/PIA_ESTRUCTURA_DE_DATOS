...
int obtenerMinimo(double *distancias, int *visitados, int vertices) { //buscar el vertice no visitado con menor distancia
    double min = INFINITO;
    int indice = -1;
...
void dijkstra(Grafo *grafo, int origen, int destino, int criterio) {
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        for (int v = 0; v < n; v++) {
            ...
            if (!visitados[v] && grafo->matriz[u][v].tiempoVuelo > 0) { //si el nodo no ha sido visitado y el peso es mayor a 0 
                ...
                if (distancias[u] + peso < distancias[v]) { // si la nueva ruta es mejor
    ...
    if (distancias[destino] == INFINITO) {      //si la distancia del origen a un vertice es infinito es porque no existe ruta
        printf("\nNo existe ruta.\n");
    ...
    while (anteriores[actual] != -1) { // suma mientras haya vertices anteriores
