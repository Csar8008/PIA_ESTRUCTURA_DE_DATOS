...
int validarFormatoArchivo(FILE *archivo) {
    ...
    if (fscanf(archivo, "%d", &cantidad) != 1 || cantidad <= 0) return 0;

    int tiempoValidacion;
    double costoValidacion;

    for (i = 0; i < cantidad; i++) {
        if (fscanf(archivo, " %c", &caracter) != 1 || caracter != '{') return 0;

        for (j = 0; j < cantidad; j++) {
            if (fscanf(archivo, " %d , %lf ", &tiempoValidacion, &costoValidacion) != 2) return 0;
            if (tiempoValidacion < 0 || costoValidacion < 0) return 0;
        }
...
void imprimirMatriz(Grafo *grafo) {
    ...
    for (i = 0; i <= grafo->vertices; i++) {
        for (int j = 0; j < ancho + 1; j++) printf("-");
    }
    printf("\n");
